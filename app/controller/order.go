...
// GetOrderHistory func gets the list of all orders history from the current user.
//
...
func GetOrderHistory(c *fiber.Ctx) error {
	...
	// Query the database to get all orders history for the current user
	...
	// Convert the orders to the schema.Order format
	orderList := make([]schema.Order, len(orders))
	for i, order := range orders {
		orderList[i] = convert.To[schema.Order](order)
	}
